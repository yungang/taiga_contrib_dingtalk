...
def _send_request(url, data):
    ...
    try:
        ...
        logger.error("Error sending request to dingtalk")
...
@app.task
def change_dingtalkhook(url, channel, notify_config, obj, change):
    obj_type = _get_type(obj)

    if not _check_notify_permission(notify_config, obj_type, 'change'):
        return

    template_change = loader.get_template('taiga_contrib_dingtalk/change.jinja')
    ...
    data = {"msgtype": "markdown", "markdown": {"title": "Info from Taiga", "text": ""}}
    data["markdown"]["text"] = change.user["name"] + ' ' + change_text.strip()
    data["markdown"]["title"] = change.user["name"] + ' ' + change_text.strip()

    # Get markdown fields
    if change.diff:
        template_field = loader.get_template('taiga_contrib_dingtalk/field-diff.jinja')
        ...
        for field_name, values in change.diff.items():
            if field_name in included_fields:
                ...
                data["markdown"]["text"] += " " + attachment["fallback"]
    ...
    #     getattr(settings, "DINGHOOKS_USERNAME", "Taiga"),
    #     change.user['name']
    # )
    # try:
    ...
    #     data["icon_url"] = getattr(settings, "DINGHOOKS_ICON", "https://tree.taiga.io/images/favicon.png")
    _send_request(url, data)


@app.task
def create_dingtalkhook(url, channel, notify_config, obj):
    obj_type = _get_type(obj)

    if not _check_notify_permission(notify_config, obj_type, 'create'):
        return

    template = loader.get_template('taiga_contrib_dingtalk/create.jinja')
    context = Context({"obj": obj, "obj_type": obj_type})
    create_text = template.render(context.flatten())
    data = {
        "msgtype": "markdown",
        "markdown": {
            "title": "Creator " + obj.owner.get_full_name(),
            "text": obj.owner.get_full_name() + ' ' + create_text.strip()
        }
    }
    ...
    if obj_type == "wikipage":
        ...
        data['markdown']['text'] += ' ' + description

    _send_request(url, data)


@app.task
def delete_dingtalkhook(url, channel, notify_config, obj, change):
    obj_type = _get_type(obj)

    if not _check_notify_permission(notify_config, obj_type, 'delete'):
        return

    template = loader.get_template('taiga_contrib_dingtalk/delete.jinja')
    context = Context({"obj": obj, "obj_type": obj_type})
    delete_text = template.render(context.flatten())
    data = {
        "msgtype": "markdown",
        "markdown": {
            "title": change.user["name"] + " delete",
            "text": change.user["name"] + " " + delete_text.strip()
        }
    }
...
@app.task
def test_dingtalkhook(url, channel):
    data = {
        "msgtype": "text",
        "text": {
